    ...
    ~H"""
      <div x-data="{open: true}" class="bg-white">
        <div class="flex relative items-center hover:bg-gray-100 px-2">
          ...
          <h2
            class="cursor-pointer flex-1 p-2 text-gray-700 text-sm"
            :on-click={{ "selectValue", target: "##{parent_id}" }}
            phx-value-select={{ Jason.encode!(%{name: assigns["name"], value: assigns["value"]}) }}
          >
            {{ assigns["name"] }}
          </h2>
        </div>
